...
function initializeApplication() {
  ...
  var getPopularSpots = function getPopularSpots(center, callback) {
    ...
    request.open('GET', foursquare.venues + foursquare.explore + foursquare.credentials + foursquare.version + coordinates + query, true);
  ...
  var getVenueDetails = function getVenueDetails(venueId, callback) {

    var request = new XMLHttpRequest();
    request.open('GET', foursquare.venues + venueId + '?' + foursquare.credentials + foursquare.version, true);
  ...
  var showInfo = function showInfo(venue, infowindow) {
    // Check to make sure the infowindow is not already opened on this marker.
    if (infowindow.marker != venue.marker) {

      var content = '<div id="content" class="content">' + '<h1 class="content__heading" data-bind="text: venueList()[' + venue.index + '].name"></h1>' + '<div class="image-slider" data-bind="foreach: venueList()[' + venue.index + '].photoUrls">' + '<div class="image-slider__image-container">' + '<img style="max-width:300px;max-height:200px" class="image-slider__image" data-bind="attr: {src: $data}" alt="">' + '</div>' + '</div>' + '</div>';
  ...
  searchBox.addListener('places_changed', function () {
    ...
    // calculate distance
    var distance = google.maps.geometry.spherical.computeDistanceBetween(places[0].geometry.location, map.getCenter());
...
define(['exports','require'],factory);}else if((typeof exports==='undefined'?'undefined':_typeof(exports))==='object'&&(typeof module==='undefined'?'undefined':_typeof(module))==='object'){// [2] CommonJS/Node.js
